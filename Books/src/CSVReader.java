...
public class CSVReader {
    ...
    boolean isMissing(int columnIndex) {
        if (current[columnIndex].equals("") && columnIndex >= current.length)
            return true;
        return false;
    }

    boolean isMissing(String columnLabel) {
        if (current[columnLabelsToInt.get(columnLabel)].equals("") && columnLabelsToInt.get(columnLabel) >= current.length)
            return false;
        return true;
    }
    ...
    long getLong(String columnLabel) {
        System.out.println(current[columnLabelsToInt.get(columnLabel)]);
        if (isMissing(columnLabel))
           return 0;
